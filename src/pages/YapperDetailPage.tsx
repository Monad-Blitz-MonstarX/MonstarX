...
import { mockYappers, generateMockXIndexData } from '../data/mockData'
import { Yapper, XIndexDataPoint, Position } from '../types'
import XIndexChart from '../components/XIndexChart'
...
export default function YapperDetailPage() {
  ...
  const [chartData, setChartData] = useState<XIndexDataPoint[]>([])
  ...
  useEffect(() => {
    ...
    if (foundYapper) {
      ...
      // 차트 데이터 생성
      const data = generateMockXIndexData(foundYapper.id, foundYapper.xIndex)
      setChartData(data)
  ...
  const currentXIndex = chartData.length > 0 ? chartData[chartData.length - 1].value : yapper.xIndex
  const xIndexChange = yapper.xIndexChange24h
  const isPositive = xIndexChange >= 0
  ...
  const maxAmount = balance / leverage
  const entryPrice = currentXIndex
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-dark-card border border-dark-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h2 className="text-xl font-semibold text-white">X Index</h2>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-lg font-semibold ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                        {currentXIndex.toFixed(2)}
                      </span>
                      <span className={`text-sm ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                        ({isPositive ? '+' : ''}{xIndexChange.toFixed(2)}%)
                      </span>
                    </div>
                  </div>
              ...
              <div className="h-64">
                <XIndexChart data={chartData} currentValue={currentXIndex} />
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-white font-semibold">${balance.toLocaleString(2)}</span>
                  </div>
